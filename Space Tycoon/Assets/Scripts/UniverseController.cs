...
public class UniverseController : MonoBehaviour
{
    ...
    private Vector3 CalculateGravForce(Vector3 playerPos, GravBody body)
    {
        ...
        if(body.name == "Five")
        {
            Debug.Log("With Mass: " + (Universe.gravConstant * body.mass * Universe.playerMass) / (dist * dist));
            Debug.Log("With Mass: " + (Universe.gravConstant * Universe.playerMass) / (dist * dist));
        }
